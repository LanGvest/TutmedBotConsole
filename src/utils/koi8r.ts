...
function decodingError(mode: Mode): string {
	if(mode === "replacement") return "\uFFFD";
	throw new Error();
}

function encodingError(mode: Mode): number {
	if(mode === "replacement") return 0xFFFD;
	throw new Error();
}

export function decodeKoi8R(value: Uint16Array | Uint8Array | Buffer | string, options?: DecodeOptions): string {
	let mode: Mode = "replacement", index: number;
	if(options && options.mode) mode = options.mode;
	const length = value.length;
	if(typeof value === "string") {
		const bytes = new Uint16Array(length);
		for(index = 0; index < length; index++) bytes[index] = value.charCodeAt(index);
		value = bytes;
	}
	const buffer = [];
...
export function encodeKoi8R(value: string, options?: EncodeOptions): Uint16Array {
	let mode: Mode = "fatal", index: number;
	if(options && options.mode) mode = options.mode;
	const length = value.length;
	...
	for(index = 0; index < length; index++) {
		...
		if(INDEX_BY_CODE_POINT.has(codePoint)) {
			const pointer = INDEX_BY_CODE_POINT.get(codePoint) as number;
